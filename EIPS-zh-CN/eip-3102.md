 ...
 * `N_l` is the hash to the node's _left child_. `N_l` is the hash to the node's _left child_. If the node does not have a left child, then `N_l` is the empty hash `empty_hash`; `N_l` is the hash to the node's _left child_. If the node does not have a left child, then `N_l` is the empty hash `empty_hash`;
 * `N_r` is the hash to the node's _right child_. `N_r` is the hash to the node's _right child_. If the node does not have a right child, then `N_r` is the empty hash `empty_hash`; `N_r` is the hash to the node's _right child_. If the node does not have a right child, then `N_r` is the empty hash `empty_hash`;
 * the optional `N_p` is the node's _prefix_ : every key into the subtrie rooted at `N` is prefixed by this bit string;
 * `N_v` is the _value_ stored at this node. The value is **only present in leaf nodes**. `N_v` is the _value_ stored at this node. The value is **only present in leaf nodes**. The value is **only present in leaf nodes**.
