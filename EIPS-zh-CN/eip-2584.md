...
This EIP describes a four phase process to complete the conversion.
  ...
  * The third and final phase begins when the overlay trie is empty. The field holding its root is removed from the block header. The field holding its root is removed from the block header.
...
The following is changed in the execution environment:
  ...
  * Upon executing a _state read_, ϒ first searches for the address in the overlay trie. If the key can not be found there, ϒ then searches the base trie as it did at block heights h' < h₁; If the key can not be found there, ϒ then searches the base trie as it did at block heights h' < h₁;
