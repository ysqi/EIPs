...
Currently it's `600` gas + `120` gas per `32` byte word. Proposed formula is `A * ((len(input) + 8) / 64 + 1) + B`, with coefficients below Proposed formula is `A * ((len(input) + 8) / 64 + 1) + B`, with coefficients below
