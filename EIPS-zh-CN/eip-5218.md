...
In the reference implementation, once a license is revoked, all sublicenses under it are revoked. In the reference implementation, once a license is revoked, all sublicenses under it are revoked. In the reference implementation, once a license is revoked, all sublicenses under it are revoked. In the reference implementation, once a license is revoked, all sublicenses under it are revoked. This is realized in a *lazy* manner for lower gas cost, i.e., assign `active=false` only for licenses that are explicitly revoked in a `revokeLicense` function call. Therefore, `isLicenseActive` returns `true` only if all its ancestral licenses haven't been revoked. Therefore, `isLicenseActive` returns `true` only if all its ancestral licenses haven't been revoked. Therefore, `isLicenseActive` returns `true` only if all its ancestral licenses haven't been revoked. Therefore, `isLicenseActive` returns `true` only if all its ancestral licenses haven't been revoked.
