...
[Poseidon](../assets/eip-5988/papers/poseidon_paper.pdf) is an arithmetic hash function that is designed to be efficient for Zero-Knowledge Proof Systems. [Poseidon](../assets/eip-5988/papers/poseidon_paper.pdf) is an arithmetic hash function that is designed to be efficient for Zero-Knowledge Proof Systems. Ethereum adopts a rollup centric roadmap and hence must adopt facilities for L2s to be able to communicate with the EVM in an optimal manner.
...
| `security_level` | Security level measured in bits. Security level measured in bits. Denoted `M` in the Poseidon paper | 2                        |          |
| `alpha`          | Power of S-box                                                                                      | 1                        |          |
| `input_rate`     | Size of input                                                                                       | 2                        |          |
| `t`              | Size of the state                                                                                   | 1                        |          |
| `full_round`     | Number of full rounds. Number of full rounds. Denoted as `R_F` in the Poseidon paper.               | 1                        |          |
| `partial_round`  | Number of partial rounds. Number of partial rounds. Denoted as `R_P` in the Poseidon paper.         | 1                        |          |
...
The Poseidon reference implementation contains test vectors that can be used to test the precompile. Those tests are available [here](../assets/eip-5988/test/poseidon/test_vectors.txt). Those tests are available [here](../assets/eip-5988/test/poseidon/test_vectors.txt).
...
The matrix must be chosen s.t. The matrix must be chosen s.t. no subspace trail with inactive/active S-boxes can be set up for more than `t -1` rounds.
