...
## 规范
### Overview
There are two main handler classifications: L2 Contract and Off-Chain Database. These are determined based off of where the handler is deployed. The handler classifications are used to better define the different security guarantees and requirements associated with its deployment. These are determined based off of where the handler is deployed. The handler classifications are used to better define the different security guarantees and requirements associated with its deployment. These are determined based off of where the handler is deployed. The handler classifications are used to better define the different security guarantees and requirements associated with its deployment.
...
A deferred mutation can be handled in as little as two steps. However, in some cases the mutation might be deferred multiple times. A deferred mutation can be handled in as little as two steps. However, in some cases the mutation might be deferred multiple times. However, in some cases the mutation might be deferred multiple times.
...
In step 2, the client builds and performs a new request based off of the type of error received in (1). These handshakes are outlined in the sections below: These handshakes are outlined in the sections below: These handshakes are outlined in the sections below: These handshakes are outlined in the sections below:
...
In this case, the client builds a new transaction for `contractAddress` with the original `callData` and sends it to a RPC of their choice for the corresponding `chainId`. The `chainId` parameter corresponds to an L2 Solution that is EVM compatible. The `chainId` parameter corresponds to an L2 Solution that is EVM compatible. The `chainId` parameter corresponds to an L2 Solution that is EVM compatible. The `chainId` parameter corresponds to an L2 Solution that is EVM compatible.
...
The user, receiving this error, creates a new transaction for the corresponding `chainId`, and builds a transaction with the original `callData` to send to `contractAddress`. The user will have to choose an RPC of their choice to send the transaction to for the corresponding `chainId`. The user will have to choose an RPC of their choice to send the transaction to for the corresponding `chainId`. The user, receiving this error, creates a new transaction for the corresponding `chainId`, and builds a transaction with the original `callData` to send to `contractAddress`. The user will have to choose an RPC of their choice to send the transaction to for the corresponding `chainId`. The user will have to choose an RPC of their choice to send the transaction to for the corresponding `chainId`. The user will have to choose an RPC of their choice to send the transaction to for the corresponding `chainId`.
...
struct domainData {
    ...
    event L2HandlerDefaultChainIdChanged(uint256 indexed previousChainId, uint256 indexed newChainId);
    /// @notice Event raised when the contractAddress is changed for the L2 handler corresponding to chainId.
    When making changes to core variables of the handler, the corresponding event MUST be emitted. This increases the transparency associated with different managerial actions. Core variables include <code>chainId</code> and <code>contractAddress</code> for L2 solutions and <code>url</code> for Off-Chain Database solutions. The events are outlined below in the WriteDeferral Interface. This increases the transparency associated with different managerial actions. Core variables include <code>chainId</code> and <code>contractAddress</code> for L2 solutions and <code>url</code> for Off-Chain Database solutions. The events are outlined below in the WriteDeferral Interface.
    event OffChainDatabaseHandlerURLChanged(string indexed previousUrl, string indexed newUrl);
...
Existing contracts that do not wish to use this specification are unaffected. Existing contracts that do not wish to use this specification are unaffected. Existing contracts that do not wish to use this specification are unaffected. Existing contracts that do not wish to use this specification are unaffected. Clients can add support for Cross Chain Write Deferrals to all contract calls without introducing any new overhead or incompatibilities.

Contracts that require Cross Chain Write Deferrals will not function in conjunction with clients that do not implement this specification. Attempts to call these contracts from non-compliant clients will result in the contract throwing an exception that is propagated to the user. Attempts to call these contracts from non-compliant clients will result in the contract throwing an exception that is propagated to the user. Attempts to call these contracts from non-compliant clients will result in the contract throwing an exception that is propagated to the user. Attempts to call these contracts from non-compliant clients will result in the contract throwing an exception that is propagated to the user.
