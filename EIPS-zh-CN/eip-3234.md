...
The `maxFlashLoan` function MUST return the maximum loan possible for each `token`. The `maxFlashLoan` function MUST return the maximum loan possible for each `token`. If a `token` is not currently supported `maxFlashLoan` MUST return 0, instead of reverting.
