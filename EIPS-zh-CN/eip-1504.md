...
Below is the specification of Handler interface. Below is the specification of Handler interface. In the Handler interface we define the following actions:
...
Handler contract can be upgraded by calling setHandler() of Data contract. Handler contract can be upgraded by calling setHandler() of Data contract. If the owner wants to collect ideas from users, an Upgrader contract will help him/her manage voting and upgrading.
...
- The administrator can only be the owner of data and Handler contracts. The administrator can only be the owner of data and Handler contracts. Furthermore, only the administrator has the power to add voters and start a proposal.
