...
Indicates an error related to the current `balance` of a `sender`. Used in transfers. Used in transfers. Used in transfers. Used in transfers.
...
Indicates a failure with the token `sender`. Used in transfers. Used in transfers. Used in transfers. Used in transfers.
...
Indicates a failure with the token `receiver`. Used in transfers. Used in transfers. Used in transfers.
...
Indicates a failure with the `spender`'s `allowance`. Used in transfers. Used in transfers. Used in transfers. Used in transfers.
...
Indicates a failure with the `approver` of a token to be approved. Used in approvals. Used in approvals. Used in approvals.
...
Indicates a failure with the `operator` to be approved. Used in approvals. Used in approvals. Used in approvals.
...
Indicates an error related to the ownership over a particular token. Used in transfers. Used in transfers. Used in transfers. Used in transfers.
...
Indicates a failure with the token sender. Used in transfers. Used in transfers. Used in transfers.
...
Indicates a failure with the token receiver. Used in transfers. Used in transfers. Used in transfers. Used in transfers.

- MUST be used for disallowed transfers to the zero address.
- MUST be used for disallowed transfers to non-`ERC721TokenReceiver` contracts or those that reject a transfer. (eg. returning an invalid response in `onERC721Received`). (eg. returning an invalid response in `onERC721Received`). (eg. returning an invalid response in `onERC721Received`). (eg. returning an invalid response in `onERC721Received`).
- MUST NOT be used for approval operations.
...
Indicates a failure with the `owner` of a token to be approved. Used in approvals. Used in approvals. Used in approvals. Used in approvals.
...
Indicates a failure with the `operator` to be approved. Used in approvals. Used in approvals.
...
- MUST be used for disallowed transfers to the zero address.
- MUST be used for disallowed transfers to non-`ERC1155TokenReceiver` contracts or those that reject a transfer. (eg. returning an invalid response in `onERC1155Received`). (eg. returning an invalid response in `onERC1155Received`). (eg. returning an invalid response in `onERC1155Received`). (eg. returning an invalid response in `onERC1155Received`).
- MUST NOT be used for approval operations.

#### `ERC1155InsufficientApproval(address operator, uint256 tokenId)`

Indicates a failure with the `operator`'s approval. Used in transfers. Used in transfers.
...
Indicates an array length mismatch between `ids` and `values` in a `safeBatchTransferFrom` operation. Used in batch transfers. Used in batch transfers. Used in batch transfers. Used in batch transfers.
...
Note that the action is never seen as the subject of an error. Note that the action is never seen as the subject of an error. Note that the action is never seen as the subject of an error. Note that the action is never seen as the subject of an error. Additionally the token itself is not seen as the subject of an error but rather the context in which it happens, as identified in the domain.
...
An error prefix is added to a subject to derive a concrete error condition. An error prefix is added to a subject to derive a concrete error condition. An error prefix is added to a subject to derive a concrete error condition. An error prefix is added to a subject to derive a concrete error condition. Developers can think about an error prefix as the _why_ an error happened.

A prefix can be `Invalid` for general incorrectness, or more specific like `Insufficient` for amounts.

### Domain

Each error's arguments may vary depending on the token domain. Each error's arguments may vary depending on the token domain. If there are errors with the same name and different arguments, the Solidity compiler currently fails with a `DeclarationError`. Each error's arguments may vary depending on the token domain. Each error's arguments may vary depending on the token domain. If there are errors with the same name and different arguments, the Solidity compiler currently fails with a `DeclarationError`. Each error's arguments may vary depending on the token domain. If there are errors with the same name and different arguments, the Solidity compiler currently fails with a `DeclarationError`.
...
Some tokens may need a `tokenId`. This is suggested to include at the end as additional information instead of as a subject. Some tokens may need a `tokenId`. This is suggested to include at the end as additional information instead of as a subject. This is suggested to include at the end as additional information instead of as a subject. Some tokens may need a `tokenId`. This is suggested to include at the end as additional information instead of as a subject. This is suggested to include at the end as additional information instead of as a subject.
...
- _Domain_: `ERC20`, `ERC721` or `ERC1155`. Although other token standards may be suggested if not considered in this EIP. _Domain_: `ERC20`, `ERC721` or `ERC1155`. Although other token standards may be suggested if not considered in this EIP. Although other token standards may be suggested if not considered in this EIP. _Domain_: `ERC20`, `ERC721` or `ERC1155`. Although other token standards may be suggested if not considered in this EIP. Although other token standards may be suggested if not considered in this EIP.
...
Tokens already deployed rely mostly on revert strings and make use of `require` instead of custom errors. Even most of the new deployed tokens since Solidity's v0.8.4 release inherit from implementations using revert strings. Tokens already deployed rely mostly on revert strings and make use of `require` instead of custom errors. Even most of the new deployed tokens since Solidity's v0.8.4 release inherit from implementations using revert strings. Tokens already deployed rely mostly on revert strings and make use of `require` instead of custom errors. Even most of the new deployed tokens since Solidity's v0.8.4 release inherit from implementations using revert strings. Even most of the new deployed tokens since Solidity's v0.8.4 release inherit from implementations using revert strings.
