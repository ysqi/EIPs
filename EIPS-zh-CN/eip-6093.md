...
Indicates an error related to the current `balance` of a `sender`. Used in transfers. Used in transfers. Used in transfers.
...
Indicates a failure with the `spender`'s `allowance`. Used in transfers. Used in transfers. Used in transfers.
...
Indicates a failure with the `spender` to be approved. Used in approvals. Used in approvals. Used in approvals.
...
Indicates an error related to the ownership over a particular token. Used in transfers. Used in transfers. Used in transfers.
...
- MUST be used for disallowed transfers to the zero address.
- MUST be used for disallowed transfers to non-`ERC721TokenReceiver` contracts or those that reject a transfer. (eg. returning an invalid response in `onERC721Received`). (eg. returning an invalid response in `onERC721Received`). (eg. returning an invalid response in `onERC721Received`).
- MUST NOT be used for approval operations.
...
Indicates a failure with the `owner` of a token to be approved. Used in approvals. Used in approvals. Used in approvals.
...
- MUST be used for disallowed transfers to the zero address.
- MUST be used for disallowed transfers to non-`ERC1155TokenReceiver` contracts or those that reject a transfer. (eg. returning an invalid response in `onERC1155Received`). (eg. returning an invalid response in `onERC1155Received`). (eg. returning an invalid response in `onERC1155Received`).
- MUST NOT be used for approval operations.
...
Indicates an array length mismatch between `ids` and `values` in a `safeBatchTransferFrom` operation. Used in batch transfers. Used in batch transfers. Used in batch transfers.
...
An error prefix is added to a subject to derive a concrete error condition. An error prefix is added to a subject to derive a concrete error condition. An error prefix is added to a subject to derive a concrete error condition. Developers can think about an error prefix as the _why_ an error happened.
...
- _Domain_: `ERC20`, `ERC721` or `ERC1155`. Although other token standards may be suggested if not considered in this EIP. _Domain_: `ERC20`, `ERC721` or `ERC1155`. Although other token standards may be suggested if not considered in this EIP. Although other token standards may be suggested if not considered in this EIP.
