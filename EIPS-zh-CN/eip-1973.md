...
### Definitions
 ...
 `participantMask`: is used to keep track of a `msg.sender` (participant) rewards over time. `participantMask`: is used to keep track of a `msg.sender` (participant) rewards over time. When a `msg.sender` joins or leaves the ecosystem, the player mask is updated
 ...
 `blockFreezeInterval`: the number of blocks that need to pass until the next mint. E.g. `blockFreezeInterval`: the number of blocks that need to pass until the next mint. E.g. if set to 50 and 'n' tokens were minted at block 'b', the next 'n' tokens won't be minted until 'b + 50' blocks have passed
...
`Step 3` : call trigger() from Alice / Bob's account. `Step 3` : call trigger() from Alice / Bob's account. 65 blocks are passed, hence 65 SIM tokens are minted. The RM is 32500000000000000000 The RM is 32500000000000000000
...
`Step 6` : call trigger() from Alice / Bob's / Satoshi / Vitalik account. 101 blocks are passed, hence 101 SIM tokens are minted. The RM is 57750000000000000000 101 blocks are passed, hence 101 SIM tokens are minted. The RM is 57750000000000000000
