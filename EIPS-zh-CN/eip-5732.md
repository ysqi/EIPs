...
4. The `timePoint` return value of `commitFrom` is RECOMMENDED to use `block.timestamp` or `block.number` or a number that indicates the ordering of different commitments. When `commitFrom` is being called. When `commitFrom` is being called. When `commitFrom` is being called.
...
1. Do not use the reference implementation in production. It is just for demonstration purposes. It is just for demonstration purposes. It is just for demonstration purposes.
...
4. Users must NEVER reuse a used `secret_salt`. It's recommended for client applications to warn users who attempt to do so. It's recommended for client applications to warn users who attempt to do so. It's recommended for client applications to warn users who attempt to do so.
