...
* Funding and payment traceability is enhanced bringing the initiation into the ledger. Funding and payment traceability is enhanced bringing the initiation into the ledger. All payment stat s can be stored on chain. Funding and payment traceability is enhanced bringing the initiation into the ledger. All payment stat s can be stored on chain. Funding and payment traceability is enhanced bringing the initiation into the ledger. All payment stat s can be stored on chain.
...
Emitted when a token holder, orderer,  has cancelled a funding request. Emitted when a token holder, orderer,  has cancelled a funding request. This can only be done if the operator hasn't put the funding order in process. Emitted when a token holder, orderer,  has cancelled a funding request. This can only be done if the operator hasn't put the funding order in process. Emitted when a token holder, orderer,  has cancelled a funding request. This can only be done if the operator hasn't put the funding order in process.
...
Funding instruction format is open. Funding instruction format is open. ISO payment standard like is a good start point, Funding instruction format is open. ISO payment standard like is a good start point,
