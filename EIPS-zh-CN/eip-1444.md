...
A fallback `Localization` with all keys filled MUST be available. A fallback `Localization` with all keys filled MUST be available. If the user-specified `Localization` has not explicitly set a loalization (ie. `textFor` returns `""`), the `LocalizationPreferences` MUST redelegate to the fallback `Localization`.
...
Registers a user's preferred `Localization`. Registers a user's preferred `Localization`. The registering user SHOULD be considered `tx.origin`.
