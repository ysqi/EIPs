...
A new reserved address is specified at `x`, in the range used for precompiles. A new reserved address is specified at `x`, in the range used for precompiles. When a transaction is sent to this address from an externally owned account, the payload of the transaction is treated as EVM bytecode, and executed with the signer of the transaction as the current account. For clarity: For clarity:
 ...
 - There is no code associated with the precompile address. There is no code associated with the precompile address. `CODE*` and `EXTCODE*` opcodes behave the same as they do for any empty address.
 - `CALLDATA*` opcodes operate on the transaction payload as expected.
 - `SLOAD` and `SSTORE` operate on the storage of the EOA. `SLOAD` and `SSTORE` operate on the storage of the EOA. As a result, an EOA can have data in storage, that persists between transactions.
