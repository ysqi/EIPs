...
The `text` data may be any arbitrary UTF-8 string. If the key is not present, the empty string must be returned. The `text` data may be any arbitrary UTF-8 string. If the key is not present, the empty string must be returned. If the key is not present, the empty string must be returned. The `text` data may be any arbitrary UTF-8 string. If the key is not present, the empty string must be returned. If the key is not present, the empty string must be returned.
...
Service Keys must be made up of a *reverse dot notation* for a namespace which the service owns, for example, DNS names (e.g. `.com`, `.io`, etc) or ENS name (i.e. `.eth`). Service Keys must contain at least one dot. Service Keys must contain at least one dot. Service Keys must contain at least one dot. Service Keys must contain at least one dot.
