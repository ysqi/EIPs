...
This API specification contains only three methods: one for **status**, one for **listing the available keys**, and one to **produce a signature**. There are no methods for authentication, key management, nor transport encryption. There are no methods for authentication, key management, nor transport encryption.
...
From an API pipeline view, we have two nodes: The validator client (1) that makes requests to the remote signer (2). A more sophisticated chain can be built by introducing elements between these two nodes. Either by setting up reverse proxy services, or by adding plugins to the remote signer implementation. A more sophisticated chain can be built by introducing elements between these two nodes. Either by setting up reverse proxy services, or by adding plugins to the remote signer implementation.
...
There are several ways to store secret keys, namely Hardware Security Modules (HSM), Secrets management applications (e.g. Hashicorp Vault), cloud storage with tight private network ACL rules, or even raw files in a directory. In general the remote signer implementers will abstract the storage medium from the HTTP API. In general the remote signer implementers will abstract the storage medium from the HTTP API.
...
| An attacker can repudiate a sent message.                                                  | Implement logging in the signer. Implement logging in the signer. Enhance it by sending logs to a syslog box.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
