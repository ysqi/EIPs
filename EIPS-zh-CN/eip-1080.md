...
Reports the `msg.sender`'s account as being not being lost. Reports the `msg.sender`'s account as being not being lost. MUST trigger the `AccountClaimedLostCanceled` event. Reports the `msg.sender`'s account as being not being lost. MUST trigger the `AccountClaimedLostCanceled` event.
...
A record of an account being frozen. A record of an account being frozen. MUST trigger when an account is frozen. A record of an account being frozen. MUST trigger when an account is frozen.
