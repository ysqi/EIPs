...
## Simple Summary
Money streaming represents the idea of continuous payments over a finite period of time. Block numbers are used as a proxy of time to continuously update balances. Block numbers are used as a proxy of time to continuously update balances.
...
3. The payee is able to withdraw money from the contract based on its ongoing solvency. The payee is able to withdraw money from the contract based on its ongoing solvency. That is: `payment rate * (current block height - starting block height)`
...
MUST allow senders to create multiple streams in parallel. MUST allow senders to create multiple streams in parallel. SHOULD not accept Ether and only use ERC20-compatible tokens.
