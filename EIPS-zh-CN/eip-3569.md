...
The sealed metadata JSON file MAY contain metadata for many different tokens. The top level keys of the JSON object MUST be token IDs. The top level keys of the JSON object MUST be token IDs.
...
There is no backwards compatibility with existing standards. There is no backwards compatibility with existing standards. This is an extension which could be added to existing NFT standards.
