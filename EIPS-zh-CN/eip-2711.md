...
A user can set `ValidUntil` to a very large number which effectively makes it non-expiring.  A user can set `ValidUntil` to a very large number which effectively makes it non-expiring.  By making `ValidUntil` optional, we can save some bytes on the wire by allowing such transcations to simply have a `0` (1 byte in RLP) value for this field.
...
- The unix time is generally available in most settings, even on a computer which is offline. The unix time is generally available in most settings, even on a computer which is offline. This means that even a setup where blockchain information is unavailable, the party signing a transaction can generate a transaction with the desired properties.
