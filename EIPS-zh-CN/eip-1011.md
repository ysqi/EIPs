...
* **epoch**: The span of blocks between checkpoints. Epochs are numbered starting at the hybrid casper fork, incrementing by one at the start of each epoch. Epochs are numbered starting at the hybrid casper fork, incrementing by one at the start of each epoch.
...
* When applying `vote` transactions to `CASPER_ADDR` to vm state:
  ...
  * Do not count gas of `vote` toward the normal `gas_used`. Do not count gas of `vote` toward the normal `gas_used`. For all `vote` transaction receipts, cumulative gas used is equal to last non-`vote` transaction receipt
...
##### Join Fork
The ability to manually join a fork specified by a blockhash. The ability to manually join a fork specified by a blockhash. A suggested implementation is `--join-fork BLOCKHASH` where the client automatically sets the head to the block defined by`BLOCKHASH` and locally finalizes it.
...
The finality gadget is designed to minimize changes across clients. The finality gadget is designed to minimize changes across clients. For this reason, FFG is implemented within the EVM, so that the contract byte code encapsulates most of the complexity of the fork.
...
##### Join Fork
This setting is to be used by new clients that are syncing the network for the first time. Due to weak subjectivity, a blockhash should be supplied to successfully sync the network when initially starting a node. Due to weak subjectivity, a blockhash should be supplied to successfully sync the network when initially starting a node.
