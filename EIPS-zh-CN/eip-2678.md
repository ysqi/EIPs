...
The `algorithm` used to generate the corresponding hash. The `algorithm` used to generate the corresponding hash. Possible algorithms include, but are not limited to `sha3`, `sha256`, `md5`, `keccak256`. The `algorithm` used to generate the corresponding hash. Possible algorithms include, but are not limited to `sha3`, `sha256`, `md5`, `keccak256`.
...
#### Contract Alias
This is a name used to reference a specific [Contract Type](#contract-type). This is a name used to reference a specific [Contract Type](#contract-type). Contract aliases **must** be unique within a single [Package](#package). This is a name used to reference a specific [Contract Type](#contract-type). Contract aliases **must** be unique within a single [Package](#package).
