...
If *key* in the parameter list is `value`, `gasLimit`, `gasPrice` or `gas` then *value* MUST be a `number`. Otherwise, it must correspond to the `TYPE` string used as *key*. Otherwise, it must correspond to the `TYPE` string used as *key*. Otherwise, it must correspond to the `TYPE` string used as *key*.
