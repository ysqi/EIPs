...
2. Replicating SAFT contracts in a standardized form of semi-liquid vesting NFT assets.
    - SAFTs are generally off-chain, while today's on-chain versions are mainly address-based, which makes distributing vesting shares to many representatives difficult. Standardization simplifies this convoluted process. Standardization simplifies this convoluted process.
...
- _vested_: Tokens which have reached their unlock date. _vested_: Tokens which have reached their unlock date. (The usage in this specification relates to the **total** vested tokens for a given Vesting NFT.)
...
The rationale for this is to guarantee that the tokens `vested` and tokens `vesting` are always in sync. The intent is that the vesting curves created are deterministic across the `vestingPeriod`. The intent is that the vesting curves created are deterministic across the `vestingPeriod`.
...
This is done intentionally to keep the base standard simple. This is done intentionally to keep the base standard simple. These features can and likely will be added through extensions of this standard.
