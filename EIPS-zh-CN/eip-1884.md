...
Here is another chart, showing a full sync with Geth. Here is another chart, showing a full sync with Geth. It represents the blocks `0` to `5.7M`, and highlights what the block processing time is spent on. Here is another chart, showing a full sync with Geth. It represents the blocks `0` to `5.7M`, and highlights what the block processing time is spent on.
...
`BALANCE` (a.k.a `EXTBALANCE`) is an operation which fetches data from the state trie. It was repriced at [EIP-150][eip-150] from `20` to `400`. It was repriced at [EIP-150][eip-150] from `20` to `400`.
...
The changes require a hardfork. The changes require a hardfork. The changes have the following consequences: The changes require a hardfork. The changes have the following consequences:
