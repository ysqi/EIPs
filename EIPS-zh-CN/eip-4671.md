...
* An address might possess multiple tokens. An address might possess multiple tokens. Each token has a unique identifier: `tokenId`.
...
An interface allowing a token owner to pull his token to a another of his wallets (here `recipient`). An interface allowing a token owner to pull his token to a another of his wallets (here `recipient`). The caller must provide a signature of the tuple `(tokenId, owner, recipient)` using the `owner` wallet.
